...
class proper_nouns
{
	...
	******************************/
	public function set_conjunctions($arr, $type = "start"){
		if(!is_array($arr))
		{
			$this->conjunctions[$type] = array(strtolower($arr));
		}
		else
		{
			$new = array();
	...
	public function set_symbols($arr){
		if(!is_array($arr))
		{
			$this->conjunctions = array(strtolower($arr));
		}
		else
		{
			$new = array();
	...
	public function set_ignore($arr){
		if(!is_array($arr))
		{
			$this->ignore = array(strtolower($arr));
		}
		else
		{
			$new = array();
	...
	public function set_punctuation($arr){
		if(!is_array($arr))
		{
			$this->punctuation = array(strtolower($arr));
		}
		else
		{
			$new = array();
	...
	public function stop_words($arr){
		if(!is_array($arr))
		{
			$this->stop_words = array(strtolower($arr));
		}
		else
		{
			$new = array();
	...
	******************************/
	...
	private function ucfirst($str, $encoding = "UTF-8", $lower_str_end = false){
		if (!function_exists('ucfirst')) {
			$first_letter = strtoupper(substr($str, 0, 1, $encoding), $encoding);
			$str_end = "";
			if ($lower_str_end) {
				$str_end = strtolower(substr($str, 1, strlen($str, $encoding), $encoding), $encoding);
			}
			else {
				$str_end = substr($str, 1, strlen($str, $encoding), $encoding);
			}
	...
	private function get_candidates($words, $text){
		...
		{
			if(trim($key) != "" && $key == $this->ucfirst($key) && !in_array($key, $proper))
			{
				//acronyms
				...
				else
				{
					...
					if($this->strict)
					{
						...
						{
							if(strtolower($key) == strtolower($val) && $val != $this->ucfirst($val))
							{
								$stop = true;
								break;
							}
					...
					if(!$stop)
					{
						...
						foreach($parts as $cnt => $part)
						{
							...
							{
								$cut = 1;
								while(strlen($part)-$cut >= 0 && in_array(substr($part, strlen($part)-$cut, 1), $this->ignore))
								{
									$cut++;
								}
								if(strlen($part)-$cut > 0 && !in_array(substr($part, strlen($part)-$cut, 1), $this->punctuation))
								{
									$proper[] = $key;
									$stop = true;
								}
								else if(!$this->strict)
								{
									$cut++;
									$str = "";
									while(strlen($part)-$cut >= 0 && (!in_array(substr($part, strlen($part)-$cut, 1), $this->ignore) && !in_array(substr($part, strlen($part)-$cut, 1), $this->punctuation)))
									{
										$str = substr($part, strlen($part)-$cut, 1).$str;
										$cut++;
									}
									if(in_array(strtolower($str), $this->conjunctions["dot"]))
									{
										$proper[] = $key;
										$stop = true;
									}
			...
			for($j = 0; $j < sizeof($words); $j++)
			{
				if($proper[$i] == $words[$j])
				{
					...
					for($k = $j-1; $k >= 0; $k--)
					{
						...
						if(in_array(strtolower($words[$k]), $this->conjunctions["start"]))
						{
							if($start != "")
							{
						...
						else if(in_array(strtolower($words[$k]), $this->conjunctions["middle"]))
						{
							$start = $words[$k]." ".$start;
						}
					...
					{
						//if conjunction can be used in the middle, we wait and see if it will be surrounded
						if(in_array(strtolower($words[$k]), $this->conjunctions["middle"]))
						{
							$end = $words[$k]." ".$end;
						}
						//if it's another proper noun - add it
	...
	******************************/
	public function get($text){
		...
		if(!empty($this->stop_words))
		{
			...
			{
				if(!in_array(strtolower($val), $this->stop_words))
				{
					$new[] = $val;
				}
			}
